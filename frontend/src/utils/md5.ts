// Minimal MD5 implementation for login hashing (no external deps).
// Adapted for browser/TS usage; returns lowercase hex string.

function rotl(x: number, c: number) {
  return (x << c) | (x >>> (32 - c))
}

function toHex32(x: number) {
  let s = ''
  for (let i = 0; i < 4; i++) {
    const b = (x >>> (i * 8)) & 0xff
    s += (b + 0x100).toString(16).slice(1)
  }
  return s
}
...
function bytesToWords(bytes: Uint8Array) {
  ...
  const words = new Array<number>(((n + 3) >> 2)).fill(0)
  for (let i = 0; i < n; i++) {
...
export function md5(input: string): string {
  const bytes = strToUtf8Bytes(String(input ?? ''))
  ...
  const bitLen = len * 8
  const totalLenWords = (((len + 8) >> 6) + 1) * 16
  while (words.length < totalLenWords) words.push(0)
  words[totalLenWords - 2] = bitLen & 0xffffffff
  words[totalLenWords - 1] = (bitLen / 0x100000000) >>> 0
  ...
  return (toHex32(a) + toHex32(b) + toHex32(c) + toHex32(d)).toLowerCase()
}
